...
export function generatePDFReport() {
  ...
  const summaryRows = [
    ...
    ["Retirement Age", readText("kpiDraw")],
    ["Funded to Year", readText("kpiYear")],
    ["Starting Balance", readText("kpiTax")],
    ["Ending Balance", readText("kpiEndBal")],
  ];

  /** @type {[string, string][]} */
  const assumptionsRows = [
    ["Starting Year", readValue("startingYear")],
    ["Subject Current Age", readValue("subjectCurrentAge")],
    ["Subject Retirement Age", readValue("subjectRetireAge")],
    ["Partner Current Age", readValue("partnerCurrentAge")],
    ["Partner Retirement Age", readValue("partnerRetireAge")],
    ["Inflation (%)", readValue("inflation")],
    ["Working Years Spending", readValue("workingYearsSpending")],
    ["Retirement Years Spending", readValue("retirementYearsSpending")],
  ];
  ...
  if (!reportWindow) {
    showToast(
      "Popup Blocked",
      "Allow popups to open the printable PDF summary.",
      "error"
    );
    return;
  }
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>Retirement Summary Report</title>
    <style>
      body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif; margin: 32px; color: #111; }
      ...
      th { width: 38%; background: #f6f6f6; }
      @media print { body { margin: 16px; } }
    </style>
  </head>
  <body>
    <h1>Retirement Summary Report</h1>
    <div class="muted">Generated by Retirement Calculator</div>

    <h2>Outcome</h2>
    <table>${tableRows(summaryRows)}</table>

    <h2>Key Assumptions</h2>
    <table>${tableRows(assumptionsRows)}</table>
  </body>
</html>`;
  ...
  reportWindow.focus();
  setTimeout(() => reportWindow.print(), 150);
